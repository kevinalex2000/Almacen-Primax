...
class ModuleClientes:
    ...
    def __init__(self,frame):
        ...
        self.frame_tabla_clientes = Frame(frame, bg= Constants.getBgColor()) ## Creamos el frame para una tabla y colocamos el color de fondo
        ...
        self.tabla_clientes.column('#0', minwidth=100, width=180, anchor='center') ## Colocamos Columna index
    ...
    def crear_formulario(self):
        ...
        #Dni
        label_dni=Label(frame_formulario, text="Dni:", bg= Constants.getBgColor())
        ...
        #Numero Celular
        label_numero_celular=Label(frame_formulario, text="Numero de Celular:", bg= Constants.getBgColor())
        ...
        #Direccion
        label_direccion=Label(frame_formulario, text="Direccion:", bg= Constants.getBgColor())
    ...
    def btn_agregar_click(self):
        if self.validar_campos_vacios():
            if self.buscar_dni_existente(self.dni.get()):
                Controls.MandarAdvertencia("El dni ya existe, por favor registre otro dni.")
            else:
    ...
    def btn_modificar_click(self):
        if self.validar_campos_vacios():
            if self.buscar_dni_existente(self.dni.get()):
                ...
                Controls.MandarAdvertencia("El dni digitado no existe.")
    ...
    def btn_eliminar_click(self):
        if self.dni.get() != "":
            ...
            Controls.MandarAdvertencia("El dni no puede estar vacio.")

    def listar_datos_tabla(self):
        self.tabla_clientes.delete(*self.tabla_clientes.get_children())
        datos = Excel.ObtenerExcel("Data/Clientes.xlsx", "Hoja1")
    ...
    def insertar_cliente(self, dni, nombre, numero_celular, direccion):
        ...
        Excel.GuardarExcel("Data/Clientes.xlsx", "Hoja1", values, datos.columns)
        self.listar_datos_tabla()
        self.limpiar_campos()
    ...
    def validar_campos_vacios(self):
        ...
        if self.dni.get() == "":
            ...
            Controls.MandarAdvertencia("El numero de celular no puede estar vacio")
        elif self.direccion.get() == "":
            Controls.MandarAdvertencia("La direccion no puede estar vacia")
        else:
            resultado = True
        return resultado
